...
class ModalIndiciosSolucao {
    ...
    attachEventListeners() {
        ...
        document.getElementById('btn_adicionar_indicio').addEventListener('click', () => {
            this.adicionar();
        });

        document.getElementById('btn_cancelar_indicio').addEventListener('click', () => {
            this.fechar();
        });
    }
    ...
    mostrarResultadosBusca(resultados) {
        ...
        resultados.forEach(item => {
            ...
            let texto = '';
            if (item.tipo === 'crime') {
                // Usar o texto_completo fornecido pela API ou construir
                texto = item.texto_completo || `${item.artigo} - ${item.descricao_artigo}`;
            } else if (item.tipo === 'art29') {
                // Usar o texto_completo fornecido pela API ou construir
                texto = item.texto_completo || `${item.inciso} - ${item.texto}`;
            } else if (item.tipo === 'rdpm') {
                // Usar o texto_completo fornecido pela API ou construir
                texto = item.texto_completo || `Art. ${item.artigo} - ${item.descricao}`;
            }
            
            div.textContent = texto;
            div.dataset.tipo = item.tipo;
            div.dataset.id = item.id;
            div.onclick = () => this.selecionarItem(item, texto);
    ...
    selecionarItem(item, texto) {
        if (!this.itensSelecionados) {
            this.itensSelecionados = [];
        }

        // Verificar se já foi selecionado
        const jaExiste = this.itensSelecionados.some(i => 
            i.id === item.id && i.tipo === item.tipo
        );

        if (jaExiste) {
            this.showToast('Aviso', 'Este item já foi selecionado', 'warning');
            return;
        }
        ...
        // Limpar busca
        document.getElementById('busca_crime_transgressao').value = '';
        document.getElementById('resultados_busca').style.display = 'none';
    }

    atualizarItensSelecionados() {
        const container = document.getElementById('itens_selecionados');
        container.innerHTML = '';
        ...
        this.itensSelecionados.forEach((item, index) => {
            ...
            div.innerHTML = `
                ...
                <button type="button" class="remove-item-custom" onclick="modalIndiciosSolucao.removerItem(${index})">
    ...
    async adicionar() {
        ...
        // Verificar se há crimes ou transgressões selecionados
        const totalSelecionados = this.crimesSelecionados.length + this.transgressoesSelecionadas.length;
        if (totalSelecionados === 0) {
            this.showToast('Erro', 'Selecione pelo menos um crime ou transgressão', 'error');
            return;
        }
...
document.addEventListener('DOMContentLoaded', function() {
    window.modalIndiciosSolucao = new ModalIndiciosSolucao();
});

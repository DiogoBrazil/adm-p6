...
- Frontend: TypeScript vanilla com templates HTML em strings.
- Build: Vite.
- Biblioteca de componentes: nenhuma; os primitivos ficam em `src/dom.ts`.
- CSS: folha global vanilla em `src/styles.css`.

## Primitivos de interface — `src/dom.ts`

Helpers compartilhados para botões, tabelas, paginação, notificações, exportação, formatação e impressão.

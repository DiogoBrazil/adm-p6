...
from app.routers import registrar_todas_rotas

class _DatabaseManagerLegacy:
...
# @eel.expose  # FUNÇÃO AUXILIAR - Pode ser migrada para catalogos router se necessário
...
# Helpers de sessão para routers
def _get_usuario_logado():
    return usuario_logado

def _set_usuario_logado(user):
    global usuario_logado
    usuario_logado = user

# Registrar todos os handlers via auto-registro
guards = {
    'login': _guard_login,
    'admin': _guard_admin
}
...
helpers = {
    'get_usuario_logado': _get_usuario_logado,
    'set_usuario_logado': _set_usuario_logado
}

# Registrar rotas inicialmente sem legacy_functions (serão registradas depois)
registrar_todas_rotas(eel, db_manager, prazos_manager, guards, helpers)

# ====================================================================
# FUNÇÕES @eel.expose MIGRADAS PARA ROUTERS - MAPA DE MIGRAÇÃO
# ====================================================================
# 
# ✅ MIGRAÇÃO COMPLETA - Todas as 73 funções # @eel.expose  # MIGRADO PARA ROUTERS foram migradas
# para routers modulares. O registro é feito via registrar_todas_rotas().
#
# MAPA DE MIGRAÇÃO:
#
# 📁 app/routers/usuarios.py (10 funções):
#    - obter_usuario_por_id, cadastrar_usuario, listar_usuarios
#    - listar_todos_usuarios, listar_encarregados_operadores
#    - obter_usuario_detalhado, atualizar_usuario, atualizar_usuario_old
#    - delete_user, verificar_admin
#
# 📁 app/routers/prazos.py (8 funções):
#    - listar_prazos_processo, adicionar_prorrogacao
#    - obter_prazos_vencendo, obter_prazos_vencidos
#    - obter_dashboard_prazos, gerar_relatorio_prazos
#    - concluir_prazo_processo, registrar_andamento_processo
#
# 📁 app/routers/andamentos.py (6 funções):
#    - adicionar_andamento, listar_andamentos, listar_andamentos_processo
#    - remover_andamento, obter_tipos_andamento, calcular_prazo_processo
#
# 📁 app/routers/indicios.py (7 funções):
#    - salvar_indicios_pm_envolvido, carregar_indicios_pm_envolvido
#    - listar_pms_envolvidos_com_indicios, remover_indicios_pm_envolvido
#    - buscar_crimes_para_indicios, buscar_rdpm_para_indicios
#    - buscar_art29_para_indicios
#
# 📁 app/routers/mapas.py (6 funções):
#    - gerar_mapa_mensal, gerar_mapa_mensal_completo, salvar_mapa_mensal
#    - listar_mapas_salvos, obter_mapa_salvo, excluir_mapa_salvo
#
# 📁 app/routers/relatorios.py (7 funções):
#    - gerar_relatorio_anual_pdf, gerar_relatorio_estatisticas_gerais
#    - gerar_relatorio_processos_por_encarregado
#    - gerar_relatorio_processos_por_tipo, gerar_relatorio_prazos_vencidos
#    - exportar_relatorio_csv, exportar_relatorio_excel
#
# 📁 app/routers/auditorias.py (5 funções):
#    - listar_auditorias, obter_auditoria_detalhada
#    - obter_auditorias_por_registro, obter_auditorias_por_usuario
#    - obter_estatisticas_auditoria
#
# 📁 app/routers/auth.py (já migrado anteriormente)
# 📁 app/routers/catalogos.py (já migrado anteriormente)
# 📁 app/routers/rdpm.py (já migrado anteriormente)
# 📁 app/routers/art29.py (já migrado anteriormente)
# 📁 app/routers/processos.py (já migrado anteriormente)
#
# TOTAL: 73 funções # @eel.expose  # MIGRADO PARA ROUTERS migradas ✅
#
# IMPORTANTE: As funções antigas abaixo foram mantidas temporariamente
# para fins de referência e troubleshooting. Elas NÃO são mais registradas
# como # @eel.expose  # MIGRADO PARA ROUTERS - apenas os routers modulares são ativos.
# ====================================================================

## MIGRADO: obter_usuario_por_id -> app/routers/usuarios.py
## MIGRADO: cadastrar_usuario -> app/routers/usuarios.py

# Continua com o resto do código legado...
# @eel.expose  # MIGRADO PARA ROUTERS
def cadastrar_usuario_old_DEPRECATED(tipo_usuario, posto_graduacao, nome, matricula, is_encarregado=False, is_operador=False, email=None, senha=None, perfil=None):
    """DEPRECATED - Cadastra novo usuário na estrutura unificada"""
...
## Processos - obter_estatistica_pads_solucoes migrado para app/routers/processos.py

## Processos - obter_estatistica_ipm_indicios migrado para app/routers/processos.py

## Processos - obter_estatistica_sr_indicios migrado para app/routers/processos.py

## Processos - obter_top10_transgressoes migrado para app/routers/processos.py

## Processos - obter_ranking_motoristas_sinistros migrado para app/routers/processos.py

## Processos - obter_estatistica_naturezas_apuradas migrado para app/routers/processos.py

## Processos - obter_estatistica_crimes_militares_ipm migrado para app/routers/processos.py

## Processos - obter_estatistica_crimes_comuns migrado para app/routers/processos.py

## Processos - obter_estatisticas_processos_andamento migrado para app/routers/processos.py
...
def _registrar_processo_wrapper(*args, **kwargs):
    """Wrapper para registrar_processo"""
    return registrar_processo(*args, **kwargs)

# Expor com o nome correto
eel._exposed_functions['registrar_processo'] = _registrar_processo_wrapper
...
def _listar_processos_wrapper():
    """Wrapper para listar_processos"""
    return listar_processos()

# Expor com o nome correto
eel._exposed_functions['listar_processos'] = _listar_processos_wrapper
...
## Processos - excluir_processo migrado para app/routers/processos.py
...
def _obter_anos_disponiveis_wrapper():
    return obter_anos_disponiveis()
eel._exposed_functions['obter_anos_disponiveis'] = _obter_anos_disponiveis_wrapper
...
def _obter_processo_wrapper(processo_id):
    """Wrapper para obter_processo"""
    return obter_processo(processo_id)

# Expor com o nome correto
eel._exposed_functions['obter_processo'] = _obter_processo_wrapper
...
def _atualizar_processo_wrapper(*args, **kwargs):
    """Wrapper para atualizar_processo"""
    return atualizar_processo(*args, **kwargs)

# Expor com o nome correto
eel._exposed_functions['atualizar_processo'] = _atualizar_processo_wrapper
...
# Registrar funções legacy que dependem de funções definidas acima
@eel.expose
def _listar_processos_com_prazos_wrapper(search_term=None, page=1, per_page=6, filtros=None):
    """Wrapper para listar_processos_com_prazos"""
    return listar_processos_com_prazos(search_term, page, per_page, filtros)

@eel.expose  
def _obter_opcoes_filtros_wrapper():
    """Wrapper para obter_opcoes_filtros"""
    return obter_opcoes_filtros()

# Expor com nomes corretos
eel._exposed_functions['listar_processos_com_prazos'] = _listar_processos_com_prazos_wrapper
eel._exposed_functions['obter_opcoes_filtros'] = _obter_opcoes_filtros_wrapper
# Nota: listar_transgressoes_wrapper é exposto diretamente com @eel.expose após a função
...
def _listar_transgressoes_wrapper(gravidade=None):
    """Wrapper para listar_transgressoes"""
    return listar_transgressoes(gravidade)

# Expor com o nome correto
eel._exposed_functions['listar_transgressoes'] = _listar_transgressoes_wrapper
...
# ========== CRIMES E CONTRAVENÇÕES - registradas via router ==========
# Handlers migrados para app/routers/catalogos.py

## RDPM - handlers migrados para app/routers/rdpm.py

## Art.29 - handlers migrados para app/routers/art29.py
...
def _listar_pms_envolvidos_com_indicios_wrapper(procedimento_id):
    """Wrapper para listar_pms_envolvidos_com_indicios"""
    return listar_pms_envolvidos_com_indicios(procedimento_id)

# Expor com o nome correto
eel._exposed_functions['listar_pms_envolvidos_com_indicios'] = _listar_pms_envolvidos_com_indicios_wrapper

...
class DatabaseManager:
    ...
    def verify_login(self, email, senha):
        ...
        cursor.execute('''
            ...
            FROM usuarios
            WHERE email = %s AND senha = %s AND ativo = 1 AND is_operador = 1
        ''', (email, senha_hash))
    ...
    def update_user(self, user_id, user_type, posto_graduacao, matricula, nome, email, senha=None, profile=None):
        ...
        try:
            ...
            if email:
                cursor.execute("SELECT id FROM usuarios WHERE email = %s AND id != %s AND ativo = 1", (email, user_id))
                if cursor.fetchone():
                    conn.close()
                    return {"sucesso": False, "mensagem": "Email já está em uso por outro usuário!"}
            
            # Verificar matrícula duplicada
            cursor.execute("SELECT id FROM usuarios WHERE matricula = %s AND id != %s AND ativo = 1", (matricula.strip(), user_id))
            if cursor.fetchone():
                conn.close()
    ...
    try:
        ...
        if termo:
            ...
            cursor.execute("""
                ...
                WHERE (inciso LIKE %s OR texto LIKE %s) AND ativo = 1
                ORDER BY 
                    CASE 
...
def buscar_indicios_por_pm(pm_envolvido_id):
    ...
    try:
        ...
        cursor.execute("""
            ...
            WHERE pm_envolvido_id = %s AND ativo = 1
        """, (pm_envolvido_id,))
...
def obter_estatistica_pads_solucoes(ano=None):
    ...
    try:
        ...
        if ano:
            where_clause += " AND strftime('%Y', data_instauracao) = %s"
            params.append(ano)
...
def obter_estatistica_ipm_indicios(ano=None):
    ...
    try:
        ...
        if ano:
            where_clause += " AND strftime('%Y', p.data_instauracao) = %s"
            params.append(ano)
...
def obter_top10_transgressoes(ano=None):
    ...
    try:
        ...
        if ano:
            where_clause_ipm += " AND strftime('%Y', p.data_instauracao) = %s"
            where_clause_sr += " AND strftime('%Y', p.data_instauracao) = %s"
            params = [ano, ano]
...
def registrar_processo(
    ...
    # Verificações específicas antes da inserção para mensagens de erro mais precisas
        ...
        cursor.execute("""
            ...
            WHERE numero = %s AND documento_iniciador = %s AND tipo_detalhe = %s AND local_origem = %s AND ano_instauracao = %s AND ativo = 1
        ...
        if numero_controle:
            cursor.execute("""
                ...
                WHERE numero_controle = %s AND documento_iniciador = %s AND tipo_detalhe = %s AND local_origem = %s AND ano_instauracao = %s AND ativo = 1
        ...
        def _resolve_user_tipo(_cursor, _id):
            if not _id:
                return None
            try:
                _cursor.execute("SELECT 1 FROM usuarios WHERE id = %s AND ativo = 1", (_id,))
                if _cursor.fetchone():
                    return 'usuario'
            except Exception:
                pass
...
def substituir_encarregado(processo_id, novo_encarregado_id, justificativa=None):
    ...
    try:
        ...
        cursor.execute("SELECT id, nome, posto_graduacao, matricula FROM usuarios WHERE id = %s AND ativo = 1", (novo_encarregado_id,))
                    ...
                    if isinstance(primeiro_item, dict):
                        ...
                        for trans_data in transgressoes_data:
                            ...
                            if tipo == 'estatuto':
                                ...
                                cursor2.execute("SELECT id, inciso, texto FROM infracoes_estatuto_art29 WHERE id = %s AND ativo = 1", (art29_id,))
                                ...
                                if art29_trans:
                                    ...
                                    if analogia_data.get('id'):
                                        cursor2.execute("SELECT id, inciso, texto FROM transgressoes WHERE id = %s AND ativo = 1", (analogia_data.get('id'),))
                                ...
                                cursor2.execute("SELECT id, inciso, texto FROM transgressoes WHERE id = %s AND ativo = 1", (trans_id,))
                        ...
                        for trans_id in transgressoes_data:
                            cursor2.execute("SELECT id, inciso, texto, gravidade FROM transgressoes WHERE id = %s AND ativo = 1", (trans_id,))
                            trans = cursor2.fetchone()
                            if trans:
                                transgressoes_selecionadas.append({
                                    'id': trans[0],
...
def obter_encarregados_procedimento(procedimento_id):
    ...
    try:
        ...
        def _buscar_usuario(user_id):
            ...
            # Busca na tabela usuarios unificada
            cursor.execute(
                "SELECT nome, posto_graduacao, matricula FROM usuarios WHERE id = %s AND ativo = 1",
                (user_id,)
            )
            u = cursor.fetchone()
            if u:
...
def obter_envolvidos_procedimento(procedimento_id):
    ...
    try:
        ...
        cursor.execute(
            "SELECT tipo_geral, nome_vitima FROM processos_procedimentos WHERE id = %s AND ativo = 1",
            (procedimento_id,)
        )
...
def atualizar_processo(
    ...
    try:
        ...
        cursor.execute("""
            SELECT id, numero, tipo_detalhe FROM processos_procedimentos
            WHERE numero = %s AND documento_iniciador = %s AND tipo_detalhe = %s AND local_origem = %s AND ano_instauracao = %s AND ativo = 1 AND id != %s
        ...
        if numero_controle:
            cursor.execute("""
                SELECT id, numero, numero_controle, tipo_detalhe FROM processos_procedimentos
                WHERE numero_controle = %s AND documento_iniciador = %s AND tipo_detalhe = %s AND local_origem = %s AND ano_instauracao = %s AND ativo = 1 AND id != %s
        ...
        try:
            ...
            if not tem_indicios_por_pm:
                ...
                for aid in art29_ids:
                    cursor.execute(
                        f"INSERT INTO procedimentos_indicios_art29 (id, procedimento_id, {col_art29}) VALUES (%s, %s, %s)",
            ...
            if pms_cadastrados and isinstance(indicios_por_pm, dict):
                ...
                for pm_id in pms_cadastrados:
                    ...
                    if not dados_indicios or not any([
                        ...
                        cursor.execute("SELECT id FROM pm_envolvido_indicios WHERE pm_envolvido_id = %s AND ativo = 1", (pm_envolvido_id,))
...
def backfill_tipos_funcoes_processo():
    ...
    try:
        ...
        def resolve_tipo(_id):
            if not _id:
                return None
            cursor.execute("SELECT 1 FROM usuarios WHERE id = %s AND ativo = 1", (_id,))
            if cursor.fetchone():
                return 'usuario'
            return None
...
def adicionar_andamento(processo_id, texto, usuario_nome=None):
    ...
    try:
        ...
        cursor.execute("""
            SELECT andamentos FROM processos_procedimentos WHERE id = %s AND ativo = 1
        """, (processo_id,))
...
def obter_estatisticas_usuario(user_id, user_type):
    ...
    try:
        ...
        cursor.execute("""
            SELECT COUNT(*) FROM processos_procedimentos 
            WHERE responsavel_id = %s AND ativo = 1 
        ...
        cursor.execute("""
            SELECT COUNT(*) FROM processos_procedimentos 
            WHERE escrivao_id = %s AND ativo = 1
        """, (user_id,))
        ...
        cursor.execute("""
            SELECT COUNT(*) FROM processos_procedimentos 
            WHERE nome_pm_id = %s AND ativo = 1 
        ...
        cursor.execute("""
            ...
            WHERE pme.pm_id = %s AND p.ativo = 1 
...
def criar_infracao_estatuto_art29(inciso, texto):
    ...
    try:
        ...
        cursor.execute("""
            ...
            WHERE UPPER(inciso) = UPPER(%s) AND ativo = 1
        """, (inciso,))
...
def editar_infracao_estatuto_art29(infracao_id, inciso, texto):
    ...
    try:
        ...
        cursor.execute("""
            SELECT id FROM infracoes_estatuto_art29 
            WHERE UPPER(inciso) = UPPER(%s) AND id != %s AND ativo = 1
        """, (inciso, infracao_id))
    ...
    try:
        ...
        query_base = """
            ...
            AND (
                ...
                (p.concluido = 0 AND p.data_instauracao < %s) OR
                -- Processos CONCLUÍDOS: concluídos especificamente no mês selecionado
                (p.concluido = 1 AND p.data_conclusao >= %s AND p.data_conclusao < %s)
            )
...
def gerar_relatorio_anual(ano):
    ...
    try:
        ...
        cursor.execute("""
            SELECT COUNT(*) FROM processos_procedimentos 
            WHERE strftime('%Y', data_instauracao) = %s
    ...
    try:
        ...
        for pm_row in pms_envolvidos:
            ...
            if indicios_registro:
                ...
                cursor.execute("""
                    ...
                    FROM pm_envolvido_crimes pec
                    JOIN crimes_contravencoes c ON c.id = pec.crime_id
                    WHERE pec.pm_indicios_id = %s AND c.ativo = 1
                """, (pm_indicios_id,))
                ...
                cursor.execute("""
                    ...
                    FROM pm_envolvido_rdpm per
                    JOIN transgressoes t ON t.id = per.transgressao_id
                    WHERE per.pm_indicios_id = %s AND t.ativo = 1
                """, (pm_indicios_id,))
                ...
                cursor.execute("""
                    ...
                    FROM pm_envolvido_art29 pea
                    JOIN infracoes_estatuto_art29 a ON a.id = pea.art29_id
                    WHERE pea.pm_indicios_id = %s AND a.ativo = 1
                """, (pm_indicios_id,))
        ...
        if not indicios["crimes"]:
            cursor.execute("""
                ...
                FROM procedimentos_indicios_crimes pic
                JOIN crimes_contravencoes c ON c.id = pic.crime_id
                WHERE pic.procedimento_id = %s AND c.ativo = 1
            """, (processo_id,))
        ...
        if not indicios["transgressoes"]:
            cursor.execute("""
                ...
                FROM procedimentos_indicios_rdpm pir
                JOIN transgressoes t ON t.id = pir.transgressao_id
                WHERE pir.procedimento_id = %s AND t.ativo = 1
            """, (processo_id,))
        ...
        if not indicios["art29"]:
            ...
            cursor.execute(f"""
                ...
                FROM procedimentos_indicios_art29 pia
                JOIN infracoes_estatuto_art29 a ON a.id = pia.{col_fk}
                WHERE pia.procedimento_id = %s AND a.ativo = 1
            """, (processo_id,))

...
| Migration nova sem acertar o preflight da importação | `importacao/00_preflight.sql` exige o número **exato** de migrations (hoje 21), para recusar um destino adiante do código tanto quanto um atrás. Uma migration nova o quebra e a migração de dados passa a recusar o destino — e **nenhum teste avisa**, porque `tests/importacao.rs` roda as 9 etapas e não o preflight | Migration nova é também: o número no preflight e as duas linhas do `importacao/README.md`. Foi assim que a `0021` chegou a produção quebrando o preflight |

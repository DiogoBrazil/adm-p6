...
| Empacotar para Windows sem declarar o subsistema | `main.rs` só tinha o `fn main`. Sem `#![cfg_attr(not(debug_assertions), windows_subsystem = "windows")]` o binário sai no subsistema **console**, e o Windows abre um terminal atrás da janela do app; fechar esse terminal mata o processo com o trabalho aberto dentro. O atributo é ignorado fora do Windows, então o `.deb` nunca mostrou nada, `cargo test` passou e o build terminou com código 0 — o defeito só aparece no PC de destino, e foi assim que o primeiro instalador entregue chegou com ele | A linha no topo do `main.rs` (é o que o template do Tauri traz, e este projeto tinha perdido). E dá para conferir sem máquina Windows: `file target/x86_64-pc-windows-msvc/release/adm-p6-tauri.exe` responde `PE32+ executable (GUI)` quando está certo e `(console)` quando não está |

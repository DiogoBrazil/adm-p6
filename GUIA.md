...
| Testes | **205** no Rust · **61** no frontend (Vitest, sobre `dom.ts` e as camadas puras das telas) |
| Módulos Rust · linhas de Rust | 13 · 11.460 |
| Arquivos de frontend · linhas de TS/CSS | 22 · 17.200 |
| Catálogos administráveis | 27 |
...
**Os envios de e-mail não são registrados** — decisão consciente desta rodada,
anotada aqui para não ser redescoberta como esquecimento. Nada no sistema
responde "este encarregado foi avisado?", nem quando, nem por quem, e num
sistema onde prazo tem peso legal essa é a pergunta que costuma vir depois.
Acrescentar a tabela é migration nova e não quebra o que já existe.

**Produção mora no Neon `adm-p6-sp`, região `sa-east-1` (São Paulo)** desde a
rodada 39. Antes era `us-east-2` (Ohio), e a mudança valeu 215 ms → **80 ms** por
ida e volta — o porquê e o que se conferiu estão na decisão 71. O projeto de
Ohio continua de pé como rollback, com as credenciais comentadas no
`.env.producao`.
...
Fora do git de propósito: tem dado pessoal de 246 militares, pela mesma razão que
...
- **Seção 3** — as 71 decisões já tomadas. Não reabra sem motivo novo.
...
| 64 | Como se fixa a ordem em que os apuratórios saem no mapa? | **Coluna `ordem` em `apuratorios`, semeada uma vez pela 0019 — não lista de siglas no código.** O pedido é que SR, IPM e PADS abram o documento quando estiverem no escopo; o resto, tanto faz. A ordem pedida não deriva de nada que já existisse: não é alfabética nem por tipo (PADS é *Processo* e vem no meio dos *Procedimentos*). Uma lista de siglas no `ORDER BY` resolveria hoje e quebraria **em silêncio** no dia em que o administrador renomeasse uma delas — sigla é apresentação (princípio 2). A carga inicial **é** por sigla, e isso é legítimo: mesmo caminho de `prazo_base_dias` (decisão 23) e dos atributos da 0007 (decisão 31); o que o princípio proíbe é o código decidir por nome **em tempo de execução**. `DEFAULT 100` mais o desempate por sigla deixam o resto exatamente como estava. A ordem vale para as **seções do PDF e as linhas da tabela da tela**, porque as duas saem do mesmo `ORDER BY` de `map_rows` (`map_print_data` o reusa) e `renderDocumentoMapa` monta as capas pela primeira ocorrência de cada apuratório — documento e tela passam a concordar. Os **checkboxes do filtro não mudam**: eles vêm do `legal_catalogs_list`, compartilhado com mais cinco telas, e nas de cadastro o primeiro da lista é o que vem pré-selecionado. A coluna entrou no catálogo administrável, então mudar a ordem amanhã é operação, não deploy — e, como `prazo_base_dias`, é campo obrigatório ao criar uma espécie. **A carga vive em dois lugares, e é de propósito.** A `0019` sozinha não bastou: `sqlx::migrate!` corre no start do app, portanto ANTES de `importacao/01_catalogos.sql`, que é quem INSERE as espécies. Num destino novo — o Neon de produção — os três `UPDATE` acharam a tabela vazia, não acertaram linha nenhuma **sem erro nenhum**, e as dez espécies nasceram depois no `DEFAULT 100`: com `ordem` empatada o desempate por sigla assumiu e o mapa voltou a abrir por CD. No banco de desenvolvimento a `0019` chegou depois da importação e pegou as linhas prontas — por isso funcionava aqui e não lá. O conserto tem as duas metades: a `0022` alcança o que já foi importado (a `0019` é imutável, checksum de `sqlx`), e a etapa 01 passou a escrever `ordem` no `INSERT` para o dado **nascer** certo. Quem afere é `tests/importacao.rs`, sobre as siglas reais: o teste de `maps_reports_repository.rs` prova o mecanismo com espécies de fixtura e por isso não pegava isto. |
| 65 | O apuratório já remetido continua sendo cobrado pelo prazo? | **Não, e é isso que o novo estado "Entregue" registra.** A situação do processo já era derivada das datas registradas, `data_remessa_*` inclusive (decisão 25) — o que faltava era a derivação completa: até aqui a coluna "Status prazo" só olhava conclusão e vencimento, e um apuratório remetido há um mês seguia escrito "Vencido há 40 dias". Quatro escolhas numa. (a) **A precedência é `Concluído` > `Entregue` > `Vencido`/`Vence em`/`Sem prazo`.** O prazo é do encarregado; registrada a remessa, ele entregou e o apuratório seguiu para julgamento ou homologação. O vencimento não some — vai para o `title` do badge, junto com a data da remessa. (b) **A remessa efetiva é `COALESCE(data_remessa_comissao, data_remessa_encarregado)`**, exposta pela `0021` como `data_remessa` na view, com `entregue` ao lado no mesmo padrão de `data_conclusao`/`concluido`. As duas colunas são alternativas da mesma etapa (decisão 47) e a `0010` já garantiu que nenhum processo tenha as duas; escolher entre elas por `permite_remessa_comissao` seria repetir em tempo de execução uma decisão que o dado já tomou. (c) **A precedência vale nos três lugares que medem prazo**: no badge (`telas/status-prazo.ts`), no filtro de Situação da listagem (`no_prazo` e `vencido` ganharam `NOT v.entregue`, e `entregue` virou opção), e na cobrança — `deadlines::report` e os **três** números de `deadlines::dashboard`, mais o `prazos_vencidos` do Painel. Tirar o entregue só das listagens deixaria o KPI dizendo oito com uma tabela de seis linhas embaixo, que é o defeito que o piso da janela veio corrigir; e tirá-lo só de `vencidos` o empurraria para "Regulares", porque a tela deriva essa faixa de `total - vencidos - proximos`. (d) **O `BALDE` de Designações NÃO muda, e a divergência é aceita.** Um quinto `WHEN` quebraria a exclusividade que faz os quatro `FILTER` somarem o total (decisão 63), então na matriz de Designações o entregue com prazo vencido continua contando em `vencidos`. As duas telas respondem perguntas diferentes: Designações mede **carga do militar**, Prazos mede **cobrança de prazo**. A cor seguiu junto: "Entregue" ficou com o amarelo (`badge--warn`) e a urgência — "Vence hoje" e "Vence em ≤5 dias" — passou a um laranja próprio, `badge--urgente`, porque dois estados diferentes na mesma coluna não podem sair da mesma cor. |
| 66 | Catálogo e configuração de apuratório só tinham "Desativar". Ganham "Excluir"? | **Ganham, e são coisas diferentes — é a decisão 54 aplicada aos cadastros.** *Desativar* tira das escolhas novas sem perder nada, e continua sendo o caminho normal e o único que serve para quem tem histórico (princípio 6). *Excluir* apaga a linha, e existe para o cadastro digitado errado — nada além disso. Quem decide é o banco: as FKs dos catálogos e as duas FKs compostas da configuração são `ON DELETE RESTRICT`, e recusam sozinhas. Nos **26 catálogos de então** não houve backend novo: `legal_catalogs_delete` existia desde a `0001`, genérico, `require_admin`, lendo o assunto antes do `DELETE` e traduzindo a violação de FK em "…já foi usado… Desative-o." — faltava só o botão, e por isso a assimetria durou. Na **configuração de apuratórios** foram dois comandos novos, e ali a tela esconde o botão quando `em_uso` (a mesma pergunta que a FK faria), o que poupa uma recusa previsível sem trocar o banco por condição de tela. O papel **responsável** não é excluível pela mesma razão que não é desativável — listagem, painel e relatórios o resolvem por `e_responsavel` —, e aqui a guarda pesa mais: desativar se desfaz reativando, excluir não. Na linha, **um só vermelho**: `danger` é da ação que não tem volta, e Desativar desceu para `outline`, como em Usuários. |
| 67 | Como se ordena artigo e inciso, que são TEXT? | **Por duas funções `IMMUTABLE` (`0023`), não por coluna administrável — porque aqui a ordem é derivada, e não escolhida.** Era ordenação alfabética, e mentia duas vezes: `transgressoes` ordenava só pelo inciso, então os 95 incisos dos Arts. 15, 16 e 17 saíam **intercalados** (o teste novo reprova o antigo com `["I", "I", "I"]` nas três primeiras linhas); e o romano em texto põe IX antes de V. No Estatuto a ordenação pelo artigo nunca chegou a ser **exercida**: só o Art. 29 está cadastrado (20 incisos), e com um artigo só não há o que ordenar errado. A coluna existe porque a `0003` funde os antigos `_art29` e `_art32` do legado — no dia em que o 32 entrar, ou um artigo de um dígito, é o número que decide, não o primeiro algarismo. `numero_do_artigo` lê a primeira sequência de dígitos (limitada a 9, senão um artigo digitado torto estoura o cast e derruba a listagem inteira) e `valor_do_romano` faz a soma posicional com subtração — este com uma porta `~ '^[IVXLCDM]+$'` por fora, porque `sum()` **ignora** nulos e sem ela `'C ú'` valeria 100. O contraste com a decisão 64 é deliberado: `apuratorios.ordem` é coluna porque lá a ordem é uma **escolha** do responsável, que não deriva de nada; aqui o número e o romano já estão escritos na linha, e uma coluna obrigaria alguém a digitar e manter 141 posições — com a próxima transgressão nascendo sem nenhuma. A regra vale nos **quatro** catálogos jurídicos, no seletor de indícios e no desempate dos rankings de enquadramento: a mesma lista não pode sair em duas ordens dependendo da tela. |
| 68 | A tela de Catálogos reparte a largura em partes iguais, e Apuratórios tem 16 colunas. O que muda? | **Largura por TIPO de coluna, em px, e piso na tabela — genérico para os 26, não exceção para um.** Repartir 100% em partes iguais dava os mesmos ~76px para "Máximo de envolvidos" e para "Nome", e era o Nome que saía "Consel…". Agora booleano vale 78px, inteiro 84, texto curto 96, referência 150, Situação 92 e Ações 128 (3×32 dos ícones + 2 vãos + padding — os 96px sugeridos cortariam), e a coluna de identificação **não declara largura**: sob `table-layout: fixed` é ela que absorve a sobra. Quem distingue "texto curto" de "texto longo" é o `centralizar` que o registro **já declarava** — sigla, artigo e inciso são centralizados; nome, texto e descrição, não —, então não entrou sinal novo para isso. O piso de Apuratórios dá **1478px**: cabe inteiro num monitor de 1920 e rola no notebook de 1366, que é o certo — `.table-wrap` já rolava, faltava o que rolar, porque `.tabela-dados--fixa` declara `min-width: 0` e a tabela sempre cabia em 100%. Duas coisas novas no registro, e as duas porque o mesmo texto servia a dois lugares com espaço diferente: `rotulo_curto` encurta só o **cabeçalho da tabela** (o formulário mantém "Permite acusação disciplinar" ao lado do campo, e o completo vai para o `title` do `<th>`), e `na_listagem` tira "Ordem no mapa" da tabela **sem** tirá-la do formulário, onde ela continua obrigatória para criar uma espécie (decisão 64). Os cabeçalhos perdem o verbo, não as palavras: "Julgamento", "Acusação disciplinar", "Remessa à comissão" — abreviatura com ponto economizaria mais e obrigaria a decifrar. |
| 69 | O gráfico vai ao papel, ou vai a tabela dele? | **A tabela, sempre — em toda tela com o alternador.** Era o gráfico: o cartão em modo "Gráfico" era congelado num PNG e impresso assim (decisão 62). A troca ganha três coisas e não perde nenhuma. A tabela é **mais completa** (o ranking plota Top 12, a tabela lista tudo); o número vira **texto pesquisável** no PDF em vez de pixels; e some a classe inteira de defeitos do `<canvas>` no papel — o retângulo preto do compositing, o desenho esticado, a imagem ainda não decodificada, o canvas impresso em duplicata ao lado do PNG. Com isso saíram `congelarGraficosParaImpressao`, `prepararGraficosParaImpressao`, `restaurarGraficosDepoisDaImpressao`, `pararEredimensionar`, `LARGURA_IMPRESSAO`, `DENSIDADE_IMPRESSAO` e o `alturaImpressao` dos seis gráficos — 174 linhas —, e o **`data:` saiu do `img-src` da CSP**, que existia só para aquele PNG (conferido no bundle: zero `data:image`). Quem faz a troca é `tabelasNoLugarDosGraficos`, e ela tira a view do gráfico **do DOM**: `hidden` não serve, porque o Chart.js escreve `display:block` inline no canvas e estilo inline vence a regra `[hidden]` do navegador. O modo que o operador escolheu é preferência dele e é devolvido no fim. A decisão 62 não foi revogada — o problema que ela resolvia deixou de existir por um caminho que não passa por canvas nenhum. |
| 70 | A folha 1 dos relatórios saía só com cabeçalho e KPIs. Por quê, e o que muda? | **O primeiro bloco de fragmentação era do tamanho dos demais, e não cabia no que sobra da folha 1 — então o bloco INTEIRO ia para a folha 2.** Não havia quebra forçada para remover: os únicos `break-after: page` do projeto são a capa do Relatório Anual e o `.mapa-pdf-*`. E `tr, th, td { break-inside: avoid }` mais `thead { display: table-header-group }` já estavam lá desde a rodada 30. O conserto é `linhasNoPrimeiroFragmentoImpressao`, que Designações já tinha (12) e as outras não — **medido, nunca estimado**: Usuários 12 (com 16 a folha 1 fica sem uma linha sequer, com 14 a última transborda a margem), Prazos 10 (12 e 14 ainda empurram tudo; 6 repete o cabeçalho no meio da folha), Contagem 15, Situação 14 e Enquadramento 5 — este último bem menor que os outros porque a descrição legal inteira faz dele a linha mais alta do relatório, e custa uma folha a mais. **O que NÃO se faz é confiar no `break-inside: avoid` do `<tr>`**: neste motor ele não segura, a linha é partida e a metade de cima **não é impressa** — 14 de 400, medidas. A segunda tabela de Prazos fica sem primeiro bloco reduzido de propósito: onde ela começa depende de quantos vencidos a primeira teve, e valor fixo acertaria um acervo e erraria o seguinte. O arnês ganhou a asserção que faltava — título e primeira linha na **mesma folha** —, e foi ela que achou o mesmo defeito nas três tabelas de Estatísticas, que ninguém tinha relatado. |
| 71 | O limite de data no campo trava o calendário. Fica assim? | **Não: o limite orienta, não prende — e nenhuma validação foi afrouxada.** A ordem cronológica (decisão 47) morava nos atributos `min`/`max`, e o navegador a aplicava travando a navegação: num apuratório instaurado em 2025, o campo de recebimento não voltava a 2024 — **nem para consertar uma instauração errada**, que é justamente quando alguém precisa daquele ano. O limite passou a `data-limite-min`/`data-limite-max`, que o navegador não conhece: o calendário navega para qualquer ano, e a regra continua cobrada por `dom.ts::mensagemDeLimiteDeData`, com a mesma mensagem, o mesmo aviso embaixo do campo e o mesmo bloqueio de envio. A troca **não é gratuita**, e é isso que a armadilha da seção 7 registra: toda a apresentação de erro pende do evento `invalid`, que só dispara em campo nativamente inválido — quem torna o campo inválido agora é o `setCustomValidity` do `revisarCampo`, e sem essa metade o formulário passaria em silêncio. As três camadas abaixo ficaram intocadas: `validar_ordem_datas` e as cinco regras "não pode ser futura" no domínio Rust, os CHECKs da `0013` e a trava de `data_recebimento` quando há prorrogação. O que o campo perdeu foi o poder de **impedir a navegação**, não o de recusar o valor. Junto veio o defeito irmão, de causa independente: o `blur()` no `change` impedia digitar o ano, porque `change` dispara no primeiro algarismo (`2` já é o ano `0002`) — ver seção 7. |
...
> **Sobre a numeração:** são 70 decisões, e a tabela tem 71 linhas — duas
> perguntas diferentes receberam o número 39. A segunda virou `39-bis` em vez
> de a numeração ser corrigida dali para a frente: as decisões são citadas pelo
> número no `CLAUDE.md`, no `README.md` e em comentário de código, e renumerar
> de 40 a 70 quebraria todas essas referências de uma vez.

---


---
| 71 | O app está lento. Trocar de provedor de banco resolve? | **Não — o que estava errado era a REGIÃO, e o provedor continua o mesmo.** A investigação começou pela Supabase, e a medição a descartou em três linhas: o projeto criado ficava em `us-east-2`, a **mesma região** do Neon, e o RTT batia igual (202–218 ms contra 208–217 ms). Provedor nunca foi a variável. O que a medição mostrou foi a distância: São Paulo responde em **74–91 ms** de onde o app roda, contra ~210 ms de Ohio — e o Neon oferece `sa-east-1`. Medido contra o banco real, com `psql`: **215 ms por ida e volta em Ohio, 80 ms em São Paulo**, 2,7×. Isso importa porque o custo de uma tela é o *número* de idas e voltas (seção 7), não o tempo de cada consulta: a ficha de um apuratório são 8 consultas em sequência (`proceedings::get_muitos`), 1,7 s lá e 0,6 s aqui, sem uma linha de código mudar. **Região de projeto Neon é imutável**, então a troca é projeto novo (`adm-p6-sp` / `little-sound-14540859`) mais cópia conferida — `pg_dump -Fc` do Neon antigo e `pg_restore --single-transaction`, e **não** a reexecução de `migrar_dados_legados.sh`, que devolveria os 163 processos do dump legado e perderia os 5 criados em produção depois dele. A conferência não foi por contagem: contagem igual não prova colação. O que prova é `md5(string_agg(nome, ORDER BY nome))` sobre os 247 militares — uma célula que muda se o ICU `pt-BR` não vier junto —, mais os checksums das 24 migrations, a ordem dos apuratórios (armadilha 0019/0022), o `permite_cadastro_vitima` das 11 espécies (armadilha 0012/0024) e o `99_conferencia.sql` inteiro comparado com o do banco antigo: idênticos, à exceção da ordem de linhas empatadas num `ORDER BY ... DESC`. **A Supabase segue possível e está documentada no `.env.producao`**, com o que ela custaria a mais: host direto só em IPv6, pooler com CA própria (o `verify-full` do rustls não a valida sem empacotar `prod-ca-2021.crt`) e o banco chamado `postgres`, que `migrar_dados_legados.sh` recusa como destino. O projeto de Ohio **não foi apagado** — é o rollback, junto com o dump. |
| 72 | Onde ficam as credenciais do servidor de e-mail? | **Numa tabela do banco, com tela de administrador — não no cofre do sistema operacional.** A conexão do banco mora no cofre de cada máquina e o instalador não embute credencial (README §8), então o caminho "óbvio" seria repetir isso. Repetir estaria errado: a conexão do banco é da MÁQUINA (cada PC alcança o Neon por conta própria), mas o remetente de e-mail é da SEÇÃO — um só, para todos. Pelo cofre, a senha de aplicativo do Gmail teria de ser digitada em cada PC, à mão, e quem esquecesse descobriria só na hora de disparar, com o encarregado esperando. Uma linha em `configuracao_email` (com `CHECK (id = 1)`, para não haver duas) serve todas as máquinas e se corrige num lugar. **O preço, declarado:** a App Password entra nos backups — que já carregam os dados pessoais de 235 militares e já ficam fora do git —, e quem girar a senha no Google precisa lembrar de trocá-la aqui. A senha **entra e não volta**: `email_config_get` devolve `tem_senha: bool` e nunca o valor, mesma regra do `database_config.rs`. E o `ConfiguracaoSmtp` do Rust **não deriva `Debug`**, pela mesma razão que `ConnectionInput` não deriva: um `panic` ou um log poriam o segredo à vista. Medido antes de decidir: as credenciais autenticam em `smtp.gmail.com` nas portas **587 (STARTTLS) e 465 (TLS implícito)** — a 587 é a de submissão padrão e ficou como padrão da tela. |
| 73 | Como o sistema sabe qual dos três avisos mandar, e para quem? | **Pelo `codigo` da mensagem, e por `COALESCE(pm.email, u.email)` no destinatário.** O tipo do aviso é `codigo` (`designacao`/`prazo_vencendo`/`prazo_vencido`), com `CHECK`, e **não** o nome: nome é apresentação e o administrador pode renomeá-lo pela tela a qualquer momento (princípio 2) — mesmo desenho de `apuratorios.codigo_extensao`. Um índice único parcial `(codigo) WHERE ativo` garante **uma** mensagem vigente por tipo, e desativar tira de uso sem apagar o texto (princípio 6). Por isso o catálogo `mensagens_email` é o único com `so_edicao: true`: sem "Novo" e sem "Excluir", porque `codigo` não é coluna editável — um "Novo" ali inseriria sem `codigo`, morreria no `NOT NULL`, e a recusa que chegaria seria "Falta preencher um campo obrigatório" sobre um campo que a tela nem mostra. **No destinatário**, um militar pode ter dois endereços: o de notificação (`policiais_militares.email`, opcional, criado pela `0025`) e o da conta de acesso (`usuarios.email`, obrigatório para quem opera). Pedir os dois seria pedir o mesmo dado duas vezes, e duas cópias divergem (princípio 4) — o de notificação vence quando existe, o da conta é a rede. E as três faltas possíveis têm **três mensagens diferentes**, porque as saídas são diferentes: sem responsável se resolve designando, sem endereço se resolve cadastrando, sem mensagem ativa se resolve em Catálogos. |
...
### 4.2 Configurabilidade — 27 catálogos + 3 tabelas de configuração
...
5. **`src/telas/processo.ts`**: a condição de renderização, e — se o campo puder esconder
   dado já gravado — o `dados.has()` de `absorverFormulario` na coleta e a regra "aparece assim mesmo quando há
...
**Fora do repositório de propósito:** tem dado pessoal de 246 militares, pela mesma razão
...
| Migration nova sem acertar o preflight da importação | `importacao/00_preflight.sql` exige o número **exato** de migrations (hoje 22), para recusar um destino adiante do código tanto quanto um atrás. Uma migration nova o quebra e a migração de dados passa a recusar o destino — e **nenhum teste avisa**, porque `tests/importacao.rs` roda as 9 etapas e não o preflight | Migration nova é também: o número no preflight e as duas linhas do `importacao/README.md`. Foi assim que a `0021` chegou a produção quebrando o preflight, e a `0022` já nasceu com as três linhas acertadas |
| Mexer no status de prazo em um lugar só | A regra vive em **quatro** derivações independentes, e não há fonte única: o badge (`telas/status-prazo.ts`), o `$9` do `FILTRO` (`proceedings/repository.rs`), o `FILTRO_REPORT` + `dashboard()` de `deadlines/repository.rs` e o `prazos_vencidos` do Painel. Mudar uma deixa a coluna dizendo "Entregue" enquanto o filtro devolve a mesma linha como "Vencido" | Mudou a regra, mudaram as quatro — e o `BALDE` de Designações é uma **quinta** que deliberadamente não acompanha (decisões 63 e 65). Os testes que amarram o par são `remessa_tira_o_apuratorio_de_vencido_sem_tira_lo_de_em_andamento` e `remessa_tira_o_processo_do_relatorio_e_dos_cartoes` |
| KPI e tabela da mesma tela saindo de consultas diferentes | Prazos e Painel desenham o cartão com uma consulta e a tabela abaixo dele com outra. Já divergiram uma vez (a janela sem piso) e voltariam a divergir com o "Entregue" fora de só uma delas — sem erro nenhum, só um número que não bate com as linhas | Toda exclusão nova entra nas duas ao mesmo tempo, e o teste afirma o par: contagem **e** ids devolvidos |
...
| Semear catálogo por migration quando quem o **insere** é a importação | `sqlx::migrate!` corre no start do app, portanto ANTES de `importacao/01_catalogos.sql`, que é quem cria as espécies. Num destino novo o `UPDATE apuratorios SET ordem = 1 WHERE lower(sigla) = 'sr'` da `0019` achou a tabela vazia: zero linhas, **sem erro e sem aviso**, e as dez espécies nasceram no `DEFAULT 100`. O mapa do Neon voltou a abrir por CD em vez de SR, e no banco local nada disso aparecia — lá a `0019` chegou depois da importação. O teste de ordem existia e passava: ele prova o mecanismo com espécies de fixtura, e nunca afirmou nada sobre as siglas reais. **E aconteceu duas vezes**: a `0012` semeia `permite_cadastro_vitima` do mesmo jeito, e no Neon as onze espécies nasceram em `false` — desligado, o atributo TIRA a seção "Ofendidos/Vítimas" do formulário, então a SR ficou sem como cadastrar ofendido enquanto exibia as 133 vítimas que a etapa 05 tinha acabado de importar. Ninguém reclamou porque o backend só recusa vítima **enviada**, e a tela nunca enviava | O valor vai no `INSERT` da etapa de importação, para o dado nascer certo; a migration (`0022`, `0024`) só alcança o que **já** foi importado, e as duas são precisas juntas. Quem afere é `tests/importacao.rs`, sobre SR/IPM/PADS de verdade e sobre as duas classes de apuratório — e a asserção é por **tipo**, não por lista de siglas, senão mente na primeira espécie nova do legado |
...
| CSP sem `ipc:` em `connect-src` | Não quebra uma tela: quebra os **95 comandos** de uma vez, porque é por aí que o IPC do Tauri v2 passa. E some no console como `Refused to connect` | `connect-src 'self' ipc: http://ipc.localhost`. Se o app abrir mudo logo na primeira tela, é isto |
...
| Esconder campo lendo o formulário pelo DOM | `FormData.get` devolve `null` tanto para "o usuário apagou" quanto para "o campo nem foi renderizado". Tratar os dois igual **apaga fato já registrado** quando a configuração muda (princípio 5) | `dados.has(campo)` separa os dois. Ver `processo.ts::absorverFormulario`; e o campo com valor gravado continua à vista, com nota, em vez de sumir |
...
| Esconder com `hidden` um elemento cujo próprio CSS declara `display` | O projeto **não tem** `[hidden]` global — só oito regras, todas com classe. Um `display: flex` no seletor vence a regra do navegador, e o elemento continua ocupando caixa. É a mesma família do canvas que seguiu sendo impresso depois de "escondido" | Quem declara `display` declara também o `[hidden]` composto: `.carregando[hidden] { display: none !important }`. Ou tira do DOM, como `graficos/index.ts::tabelasNoLugarDosGraficos` faz |
...
| Encadear dois `await` que não dependem um do outro | `desenharFormularioProcesso` esperava `carregarCatalogos()` (8 chamadas) **inteiro** antes de começar o `proceedings_get`, e nenhum dos dois precisa do outro: eram duas esperas em fila onde cabia uma. Não aparece lendo o trecho — cada `await` isolado parece necessário, e é a **ordem** entre eles que custa. Com a ficha em 8 consultas sequenciais (~720 ms), a espera dos catálogos era ~320 ms empilhados por cima, à toa | A promessa nasce antes e é esperada onde o valor é **usado**, não onde é pedida. `call` nunca rejeita (devolve `{ ok: false }`), então promessa criada e não esperada num caminho não vira rejeição solta — mas só crie a que vai ser consumida. E confira a dependência de verdade: no **detalhe** a mesma junção é impossível, porque `apuratorio_config_get` precisa do `apuratorio_id` que vem da resposta anterior |
| Subir uma chamada condicional para a onda de cima "já que é de graça" | Os 3 catálogos de acusação do formulário só são pedidos quando a espécie permite acusação, e **4 das 11 permitem**: antecipá-los desperdiçaria 3 consultas em 7 casos de 11. E a saída aparentemente óbvia — ler `permite_acusacao` do `cats.apuratorios` que já veio — é a armadilha da decisão 29: a lista de catálogos projeta só as colunas declaradas no registro de administração, e foi assim que a carta precatória parou de funcionar | Paralelize o que é **incondicional**. O que depende de uma resposta anterior continua esperando por ela, e atributo de comportamento vem de `apuratorio_config_get`, nunca da projeção do catálogo |
| Instalador antigo com o nome que se distribui | O bundler NSIS nomeia o arquivo pelo `productName` (`Gestao P6_…`, com espaço), e a receita do Windows renomeia à mão para `gestao-p6_…`. Quem constrói de novo gera o nome **com espaço**, e o arquivo de nome limpo que está ao lado é o da geração **anterior** — com o nome que a documentação manda entregar. Aconteceu: um instalador de 7/9, com o subsistema **console**, ficou dois dias ao lado do build novo, e nada na pasta dizia qual era qual | `mv -f` na mesma receita, sobrescrevendo. E confira o que vai sair: `file …/release/gestao-p6.exe` tem de dizer **(GUI)** |
| Conferir o subsistema do `.exe` pelo nome do crate | O binário leva o `mainBinaryName` do `tauri.conf.json` (`gestao-p6.exe`), não o nome do crate (`adm-p6-tauri`). Três documentos mandavam conferir `adm-p6-tauri.exe`, que **não existe**: o `file` respondia "No such file or directory", que se lê como ruído — e por isso a conferência do subsistema nunca rodou de verdade, nem depois do commit que a motivou | O nome sai do `tauri.conf.json`. E teste que não pode falhar não é teste: se o `file` não responder `PE32+ executable`, o problema é o caminho, não o binário |
| Conserto que entra num helper e deixa a cópia manual de pé | A rodada de datas corrigiu a digitação **dentro** de `dom.ts::ligarCamposDeData` — a guarda que impede o `blur()` de quem está teclando — e deu o assunto por encerrado. Só que dois formulários não usavam o helper: `#form-prorrogacao` e `#form-editar-prorrogacao` tinham o mesmo `change` escrito à mão, com `requestAnimationFrame(() => input.blur())` **sem** guarda nenhuma. O defeito seguiu vivo neles por uma rodada inteira, e é dos que não aparecem em teste: digita-se `2` no ano, o valor já fica completo como `0002`, o `change` dispara e o foco vai embora no primeiro dígito | Varra o **elemento** (`grep 'type="date"'`) e confira quem chama o helper, não quem tem o atributo certo. São 8 campos e 6 formulários, e a conta tem de fechar. Callback opcional em `ligarCamposDeData` existe para isso: um formulário que só quer o comportamento do seletor não tem desculpa para reescrever o `change` |
| Guardar posição de rolagem no `localStorage` junto com preferência | Recolhido (`adm-p6:sidebar-recolhida`) e grupos abertos são **preferência declarada** e sobrevivem à reabertura de propósito. Rolagem é onde a pessoa parou: abrir o programa com o menu no meio de Catálogos é sinal de estado velho, não de continuidade. O `preservarRolagem` lê o `scrollTop` do DOM anterior e reaplica no novo — vale para a navegação, e morre com o processo | Estado que responde "o que eu escolhi" persiste; estado que responde "onde eu estava" vive só na sessão |
| Conferir o campo de data nativo pelo `tauri dev` do Linux | O `input[type="date"]` do **WebKitGTK 2.52.6 é `MM/DD/AAAA` fixo**: medido, `2027-03-10` renderiza `03/10/2027`, e não muda com `lang="pt-BR"` no `<html>`, com `lang="en-US"` no elemento, nem com `LC_ALL` em `pt_BR.UTF-8`/`pt_BR`/`en_US.UTF-8`/`C`. O primeiro segmento vira o **mês**, que trava em 12 — quem digita 20 no que pensa ser o dia recebe 12, sem aviso. No **Edge**, que é o motor do WebView2 do app instalado, o mesmo arquivo dá `10/03/2027` e o campo vazio mostra `dd/mm/aaaa`. Ou seja: o defeito visto em dev **não existe** no app que o usuário tem, e uma rodada inteira pode ser gasta consertando o que já está certo | Meça no motor de destino. `tools/impressao/imprimir.py` imprime pelo WebKitGTK; para o lado Windows, `msedge.exe --headless --print-to-pdf` com `pdftotext` responde em segundos. É a mesma lição do `@page`, invertida: lá o Chromium mentia sobre o WebKitGTK, aqui o WebKitGTK mente sobre o Chromium |
| Tentar fixar o formato da data por `lang`, locale ou `--lang` | Nenhum dos três decide no app. O `wry` define o idioma do WebView2 **explicitamente**, de `GetUserDefaultUILanguage()`, por `options.set_language(...)` (`wry-0.55.1/src/webview2/mod.rs:330`) — propriedade dedicada do ambiente, não argumento de linha de comando. Então o formato segue o **idioma do Windows da máquina**, e `WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS=--lang=pt-BR` ou colide com essa propriedade ou é ignorado; não há como decidir sem rodar no Windows, e Tauri/wry não expõem lever para sobrescrever | Num Windows em português sai `dd/mm/aaaa` — que é o caso das máquinas da seção. Se um dia uma delas estiver em inglês, o conserto é o **idioma de exibição do Windows**, não código; e a ordem do campo sempre está visível no gabarito (`dd/mm/aaaa` vs `mm/dd/yyyy`), que é o que permite ao operador perceber |
| Declarar a regra e testá-la só em parte das fontes | `error.rs` abre dizendo que nenhuma mensagem carrega vocabulário técnico e que **toda mensagem diz o que fazer** — e testava isso para `mensagem_de_constraint` e `mensagem_de_categoria`, as duas que moram no arquivo. A terceira fonte, `AppError::Domain`, é a **maior** (82 mensagens em oito arquivos, indo verbatim para a tela, porque `message()` devolve a `String` intacta) e nunca teve teste. Derivou: 36 chegaram minúsculas e sem pontuação, 10 sem acento — `apuratorio nao encontrado`, `o anexo esta vazio`, `conteudo do anexo nao esta em base64`. Uma de cada vez, durante rodadas | Quem escreve a regra escreve o teste que a alcança **inteira**. Quando ela vive em literais espalhados, o teste que alcança é o que varre o fonte — `tests/mensagens_de_erro.rs`, no padrão de `tests/sql_prepare.rs`. E ele precisa de um **piso** (`a_varredura_alcanca_as_mensagens_de_dominio`): varredura que reconhece zero literais aprova tudo em silêncio |
| Helper de teste que converte o erro diferente do app | `tests/proceedings_repository.rs::salvar` fazia `tx.commit().await.map_err(\|e\| e.to_string())`, e o comando faz `tx.commit().await?` — que vira `AppError::Database` e passa por `message()`. São textos **diferentes**: `to_string()` devolve o PL/pgSQL cru (`apuratorio aceita no maximo 1 envolvido(s)`), `message()` devolve o que a tela mostra. Dois testes afirmavam `contains("envolvid")` e passavam lendo o texto cru, enquanto o usuário recebia "Os dados informados não atendem a uma regra do cadastro" — o helper aprovava exatamente o que o app reprovava | O helper converte pelo mesmo caminho do comando. E asserção sobre mensagem cobra a **frase do usuário**, com o número e a saída (`no máximo 1 envolvido(s)`, `informados 3`), não um radical que casa com qualquer coisa |
| Teste que prende o defeito em vez de denunciá-lo | Três asserções cobravam literalmente o que estava errado: `contains("nao encontrado")` (sem acento) e `contains("base64")` — esta última exigindo que a tela mostrasse o nome do transporte do IPC. Corrigir a mensagem **quebrou os testes**, e por um instante pareceu regressão | Asserção sobre texto de usuário afirma o que a frase tem de **fazer** — dizer o que houve e a saída —, nunca o texto exato de uma implementação que se quer poder melhorar |
| Gatilho `DEFERRABLE` que levanta `RAISE EXCEPTION` sem nome de constraint | `tg_max_envolvidos` usa `USING ERRCODE = 'check_violation'` e mais nada, então `banco.constraint()` é `None` e `error.rs` só alcança a categoria: **"Os dados informados não atendem a uma regra do cadastro. Revise os campos."**, no `commit`, sem dizer que fala de envolvidos nem qual é o limite. Cinco das onze espécies valem 1 (CD, CJ, PAD, PADE, PADS), então um segundo envolvido num PADS é rotina. O irmão `max_ocupantes` já era pré-validado no Rust com frase precisa; o de envolvidos não aparecia em `proceedings/` nem em `evidence/` | Valide **antes do SQL** — é a terceira camada que o próprio `error.rs` recomenda. O gatilho continua sendo a garantia do banco e deixa de ser o que o usuário encontra. Provar custa desligar a validação e ver o teste reprovar com a frase genérica na mão |
| Guardar credencial nova onde a do banco está | O `.env.producao` **não é lido pela aplicação** — só pelo `psql`/`pg_dump` e pelas ferramentas do `sqlx`. O instalador também não embute nada: a conexão do banco vem do cofre do SO de cada máquina. Credencial nova (o SMTP foi a primeira) precisa de lugar próprio DECIDIDO antes de implementar, e descobrir isso no fim custa a rodada inteira | Pergunte "isto é da máquina ou da seção?". Conexão de banco é da máquina; remetente de e-mail é da seção, e por isso foi para o banco (decisão 72) |
| Botão que o registro genérico oferece e a tabela recusa | O CRUD de catálogos desenha "Novo" e "Excluir" para todos os 27. Em `mensagens_email` o "Novo" insere sem `codigo` — que não é coluna editável — e morre no `NOT NULL`; a recusa que chega é "Falta preencher um campo obrigatório", **sobre um campo que a tela nem mostra**. Excluir uma das três é pior: some sem erro e o aviso correspondente para de existir | `Catalogo::so_edicao` declara isso no registro, e a tela esconde os dois botões. Caminho que só erra não se oferece — e as 27 entradas passaram a declarar o campo explicitamente, no estilo do arquivo, em vez de um padrão implícito |
| Derivar `Debug` num tipo que carrega segredo | `expect_err` de teste exige `Debug` no tipo de SUCESSO, e `ConfiguracaoSmtp` carrega a senha do servidor: derivá-lo por conveniência de teste poria o segredo ao alcance de qualquer `panic` ou log. É a mesma razão pela qual `ConnectionInput` não o deriva | `.err().expect(...)` em vez de `.expect_err(...)` — o `Option::expect` não exige `Debug` no valor descartado |
| `<input>` para texto de vários parágrafos | Um `<input type="text">` aceita o valor e **não perde nada** — mas esconde as quebras de linha, que são o formato do texto que a pessoa está editando. O corpo de um aviso viraria uma linha só, impossível de conferir | `TipoColuna::TextoLongo` e `<textarea>`. E na listagem ele precisa de teto (`LARGURA_PX.textoLongo`): sem largura fixa ele seria a coluna flexível e comeria a tabela, espremendo justamente a que identifica a linha |
| Passar `() => {}` como `aoCancelar` de `montarModal` | O helper liga Cancelar, `Esc` e o clique fora ao **`aoCancelar`**, e não ao `fechar` — de propósito, é o que permite a um formulário sujo perguntar antes de descartar. Quem passa função vazia deixa os **três** gestos sem efeito e o modal fica preso na tela, sem erro no console. Aconteceu no modal de notificação por e-mail, e os outros quatro chamadores estavam certos: `catalogos.ts` e `usuarios.ts` fecham dentro do `concluir`, o de filtros declara `const cancelar = () => modal?.fechar()` | Declare `const cancelar = () => modal?.fechar()` **antes** do `montarModal` e passe isso — o `let modal` já existe para a closure alcançá-lo. Se um dia o modal não puder fechar direto, é ali que a pergunta entra |
| Estilizar e-mail com `<style>`, ou pedir HTML ao administrador | Gmail e Outlook descartam `<style>` de forma inconsistente; só o atributo `style` inline é respeitado por todos. E o texto é editado por um sargento da Seção, não por quem programa: pedir HTML ali é pedir que ele quebre a mensagem. Imagem também não resolve — `data:` URI o Gmail descarta, e CID incha o e-mail e a maioria dos clientes bloqueia por padrão | O administrador digita **texto puro** e `email/domain.rs::montar_html` aplica a moldura no envio, com estilo inline, mandando as duas versões em `multipart/alternative`. Uma convenção só, e tolerante: parágrafo indentado vira o bloco destacado; sem indentação vira parágrafo comum. E **tudo que é variável passa por `escapar`** — o corpo vem da tela e o nome vem do banco, e um `<` solto quebraria a marcação na caixa de entrada de outra pessoa |
| Reaproveitar na prévia o HTML que vai no e-mail | A CSP do app é `style-src 'self'`, sem `unsafe-inline`, e recusa exatamente o estilo inline que o e-mail exige. A prévia mostra o **texto** — que é o que o administrador controla e o que pode sair errado (marcador digitado errado, dado do apuratório); a moldura é fixa e não é o que se confere | Uma nota na prévia dizendo que o envio leva a formatação da Seção, para a diferença não parecer defeito. E conferir a moldura de verdade se faz renderizando o HTML — `msedge.exe --headless --screenshot`, que é da mesma família do Gmail na web |
| Escolher a região do banco pelo provedor, e não pela distância | Trocar de provedor mantendo a região não muda nada: a Supabase em `us-east-2` mediu 202–218 ms, o Neon em `us-east-2` mediu 208–217 ms. É o mesmo datacenter em Ohio. O que muda é a distância — São Paulo mediu **80 ms por ida e volta contra 215 ms**, e o Neon oferece `sa-east-1` (decisão 71) | Meça o RTT antes de escolher, e meça **por região**, não por marca. `psql` com um laço de 20 `SELECT 1` na mesma sessão dá o número puro, sem handshake no meio |
| Criar o banco no projeto Neon novo sem declarar a colação | `CREATE DATABASE` pelado herda o `template1`, que na Neon é `C.UTF-8` **sem ICU**, enquanto o `admp6db` é ICU `pt-BR`. Sem isso ÉRIKA sai depois do Z, e **contagem nenhuma acusa**: as 9 tabelas batem, as 24 migrations batem, e a lista de militares sai fora de ordem | Declare por extenso (`TEMPLATE template0 … LOCALE_PROVIDER icu ICU_LOCALE 'pt-BR'`) — é o que `migrar_dados_legados.sh::opcoes_locale_de` já fazia. E confira com **uma célula**: `md5(string_agg(nome, E'\n' ORDER BY nome))` sobre `policiais_militares` muda se a colação mudar |
| `sslmode=verify-full` no `psql` do host | Falha com `root certificate file "~/.postgresql/root.crt" does not exist`. O libpq **não** usa o truststore do sistema; procura um arquivo no home — e a mensagem culpa o arquivo que falta, nunca a opção que falta. O rustls do app não tem esse problema, então a diferença só aparece na linha de comando | `?sslmode=verify-full&sslrootcert=system`. As duas metades, sempre juntas |
| Deixar o Vite vigiar `src-tauri/` | O `cargo-xwin` baixa o SDK do Windows em `target/xwin-cache/` com um symlink self-referencial de propósito (`lib/10.0.26100 -> .`), e o chokidar o segue até `ELOOP`. O `npm run tauri dev` morre **antes de abrir a janela**, com um erro que não fala de Tauri nem de Rust — e só em quem já gerou o instalador do Windows uma vez | `vite.config.ts` com `server.watch.ignored: ["**/src-tauri/**"]`. Nada em `src-tauri/` é servido ao navegador, e recompilar o Rust é trabalho do watcher do próprio `tauri dev` |
| Uma consulta por linha, dentro de um laço | O custo de uma tela não é o tempo das consultas: é o **número de idas e voltas**. Em `localhost` cada uma custa 0,2 ms e mil delas passam despercebidas por oito anos; com o banco na nuvem cada uma custa ~224 ms e as mesmas mil viram **3,8 minutos**. `map_print_data` montava as fichas do Mapa do Período uma a uma — `proceedings::get` (8 consultas) + prazos + andamentos + `evidence::list_for_proceeding`, que ainda tinha um N+1 dentro (4 consultas por envolvido). Num mês típico, 53 processos e 94 envolvidos davam `1 + 12×53 + 4×94 = 1.013` idas ao banco, e o véu de carregamento não fechava mais. Nenhum teste pegou: todos montavam **um** processo com **um** envolvido | A folha vira consulta em lote (`WHERE ... = ANY($1::uuid[])`) e a de um id só passa a ser casca fina sobre ela — o SQL fica num lugar só e a tela de processo não muda. Ver `db/lote.rs`, e `agrupar`, que preserva a ordem de chegada dentro de cada grupo (é ela que ordena envolvidos e andamentos no papel). Lista vazia **não vai ao banco**: `= ANY('{}')` responderia certo, mas comprado a 16 round-trips. Medido: 1.013 consultas → **17**, e 3,8 min → **8,8 s** contra o banco real. E teste de agrupamento precisa de **mais de um grupo**, com formatos desiguais — `mapa_em_lote_nao_mistura_as_fichas` |
| Responder ao ping por consulta desligando o `test_before_acquire` | Ele custa um round-trip por `acquire`, e com o banco remoto isso dobra o custo de quem faz muitas consultas seguidas. Desligá-lo é a resposta errada: é ele que impede entregar à consulta um socket que o servidor já derrubou — e num Postgres serverless, que suspende o compute por ociosidade, isso acontece de verdade | Peça **menos conexões**, não menos verificação: quem faz uma sequência de consultas tira uma conexão e a reusa (`pool.acquire()` uma vez, `&mut *conn` nas folhas — é para isso que as assinaturas `PgExecutor` servem). Foi o que levou `proceedings::get` de 8 pings para 1 e `map_print_data` de 17 para 1. E `min_connections` fica em **zero**: conexão aberta conta como atividade e impede a suspensão, trocando latência por consumo 24 h por dia |
| Frase de constraint escrita para uma direção que sai na outra | `error.rs::mensagem_de_constraint` tem texto próprio para `fk_designacao_apuratorio_papel`, e ele foi redigido para o **insert**: *"A função escolhida não está prevista para esta espécie. Cadastre-a…"*. A mesma constraint dispara quando se tenta **excluir** a linha de configuração que uma designação já cita — e aí a frase manda cadastrar o que já existe. É a regra certa descrevendo a situação errada, sem erro nenhum | Quem apaga intercepta `is_foreign_key_violation()` no **repositório** e devolve a frase da exclusão, antes de a rede genérica de `error.rs` opinar. É o que `legal_catalogs::repository::delete` já fazia, e o que `apuratorio_config::repository::{delete_documento, delete_papel}` passaram a fazer |
| Filtrar uma listagem pelo valor bruto quando a coluna é referência | O filtro de Catálogos comparava o termo com `linha[coluna.nome]`, e em coluna de referência isso é o **UUID**: a tabela mostrava "Art. 15" e digitar `15` não alcançava o artigo — pior, podia casar por acaso com o hexadecimal de outro UUID. Valia para todas as referências dos catálogos, e nenhuma era pesquisável | Compare com o que a tela **mostra**: `catalogos.ts::textoDaListagem`, o mesmo texto que a célula recebe — e **sem** o travessão do vazio, senão digitar "—" traz de volta toda linha em branco. A função pura é `linhasQueOTermoAlcanca`, testada em `catalogos.test.ts` |
| Confiar em `sum()` para propagar NULL | `sum()` **ignora** nulos. Numa conversão de romano letra a letra, um caractere fora de `IVXLCDM` vira `NULL` na parcela e é simplesmente pulado: `'C ú'` devolveria 100, e o inciso torto receberia posição inventada em vez de cair no `NULLS LAST` de quem ordena | A porta é explícita e fica **por fora** da soma — `valor_do_romano` só soma o que casa com `~ '^[IVXLCDM]+$'`. E a janela mora um nível abaixo do agregado, porque o PostgreSQL calcula agregação antes de janela e recusa `sum(... lead() ...)` na análise |
| Ordenar artigo ou inciso jurídico como texto | `artigo` e `inciso` são TEXT: `'Art. 5'` vai para depois de `'Art. 32'`, e o romano sai `I, II, III, IV, IX, V`. Em `transgressoes` era pior — a ordenação não incluía o artigo, e os 95 incisos dos três artigos do RDPM saíam **intercalados**. E no Estatuto o defeito estava latente: com um único artigo cadastrado (o 29), a ordenação pelo artigo nunca foi exercida por dado nenhum | `numero_do_artigo()` e `valor_do_romano()` da `0023`, nos quatro catálogos jurídicos, no seletor de indícios e no desempate dos rankings — a mesma lista não pode sair em duas ordens conforme a tela. Teste de ordenação precisa de um artigo de **um dígito**: com 15/16/17 a ordem textual acerta e o teste não prova nada |
| Fragmentar uma tabela sem medir o **primeiro** bloco | Os demais blocos têm a folha inteira; o primeiro divide o que sobra da folha 1 com o cabeçalho institucional, o título e a faixa de KPIs. Um bloco do tamanho normal não cabe ali e, como é indivisível, vai **inteiro** para a folha 2 — a folha 1 sai sem uma linha sequer de tabela, e as três asserções antigas do arnês continuam passando, porque nada se perdeu nem foi partido | `linhasNoPrimeiroFragmentoImpressao`, medido em `tools/impressao` com `--fragmento=nome:normal:primeiro`. O valor bom é o maior que mantém título e `L0001` na mesma folha, um cabeçalho por folha e nada fora das margens. A asserção `textosNaMesmaPagina` é o que impede a regressão |
| Ler uma quebra de página como "tem uma quebra forçada em algum lugar" | Neste projeto não há: os únicos `break-after: page` são a capa do Relatório Anual e o `.mapa-pdf-*`. Procurar o que não existe custa a rodada inteira, e a correção óbvia — pôr `break-inside: avoid` no `<tr>` e soltar a tabela — **reintroduz perda silenciosa de linha** | Antes de mexer em CSS de quebra, imprimir e medir. `pdftotext -f 1 -l 1` no PDF do arnês responde em um comando quais linhas caíram na folha 1 |
...
| Empacotar para Windows sem declarar o subsistema | `main.rs` só tinha o `fn main`. Sem `#![cfg_attr(not(debug_assertions), windows_subsystem = "windows")]` o binário sai no subsistema **console**, e o Windows abre um terminal atrás da janela do app; fechar esse terminal mata o processo com o trabalho aberto dentro. O atributo é ignorado fora do Windows, então o `.deb` nunca mostrou nada, `cargo test` passou e o build terminou com código 0 — o defeito só aparece no PC de destino, e foi assim que o primeiro instalador entregue chegou com ele | A linha no topo do `main.rs` (é o que o template do Tauri traz, e este projeto tinha perdido). E dá para conferir sem máquina Windows: `file target/x86_64-pc-windows-msvc/release/gestao-p6.exe` responde (o binário leva o `mainBinaryName` do `tauri.conf.json`, não o nome do crate — conferir o nome errado responde "No such file", e isso é fácil de tomar por ruído) `PE32+ executable (GUI)` quando está certo e `(console)` quando não está |
...
| `blur()` no `change` de um `input[type="date"]` | O `change` dispara assim que o valor fica **completo**, e ao digitar o ano o primeiro dígito `2` já é o ano completo `0002`: tirar o foco ali interrompe a digitação no primeiro algarismo, e o campo só aceita data escolhida no calendário. O `blur()` estava lá por um motivo real — em algumas plataformas o seletor nativo do WebView permanece aberto depois da escolha —, e ninguém percebeu o preço porque quem testa clicando nunca digita | `dom.ts::ligarCamposDeData` marca no `keydown` que a mudança veio do teclado e só fecha o seletor de quem escolheu com o mouse. O teste que pega isso precisa **teclar** antes de disparar o `change`; o que só dispara `change` aprova o defeito |
| Tirar `min`/`max` de um campo de data para destravar o calendário | Sai junto a validação: sem os atributos o campo fica nativamente válido, o evento `invalid` nunca dispara, e o formulário é enviado **em silêncio** com a data fora de ordem — só o Rust reclama, depois da ida ao banco. Toda a apresentação de erro do projeto (`instalarValidacaoAmigavel`) pende do `invalid` | O limite mora em `data-limite-min`/`data-limite-max`, que o navegador não conhece, e `dom.ts::mensagemDeLimiteDeData` o cobra. Quem torna o campo inválido é o `setCustomValidity` do `revisarCampo` — sem ele o resto da corrente não roda. Ver decisão 71 |
| `data-select-pesquisavel` no markup sem `ativarSelectsPesquisaveis` na tela | O atributo sozinho **não faz nada**: quem instancia o Tom Select é a chamada, e sem ela o campo abre um `<select>` nativo. Não há erro, não há aviso, e o markup jura o contrário — a tela de detalhe passou assim desde que nasceu, com cinco campos, incluindo o Sucessor de uma substituição entre 245 militares. Foi um operador quem percebeu, não um teste | Tela que desenha campo pesquisável chama `ativarSelectsPesquisaveis` depois do `shell()`, **no mesmo escopo** que `shell()` destrói (`#app`) — é o par que torna o redraw seguro. Para conferir: todo `data-select-pesquisavel` tem de ter uma chamada na tela que o desenha |
| Ligar o Tom Select numa tela que mexe no `<select>` por DOM nativo | Quem manda no que aparece é a **instância**, não o elemento. `select.value = "…"`, `form.reset()` e `<option>` inserida em runtime mudam o `<select>` e deixam o controle exibindo o estado anterior — silenciosamente. O caso que mais dói é a **opção histórica**: `garantirOpcaoHistorica` insere a solução desativada para o processo de 2019 continuar legível (princípio 6), e o Tom Select não a enxerga — a opção some do menu e o campo abre vazio sobre um valor que existe | `dom.ts::sincronizarSelectsPesquisaveis` depois de **qualquer** manipulação nativa; ela chama `sync()`, que relê opções e valor do original. E o foco vai por `dom.ts::focarCampo`: focar o `<select>` cru manda o cursor para o elemento que o Tom Select mantém recortado |
...
| de onde sai o "sobre o quê" de cada linha da auditoria | `audit/assunto.rs` (cabeçalho) — e `Catalogo::assunto_sql`, para os 27 catálogos |
...
| como a tela de Catálogos decide a largura de cada coluna | decisão **68**, `catalogos.ts::LARGURA_PX` / `larguraFixaPx` / `colunasFlexiveis` / `pisoDaTabela` (cabeçalhos), testados em `catalogos.test.ts` |
| por que uma coluna do formulário não aparece na tabela | `Coluna.na_listagem` e `catalogos.ts::colunasDaListagem` — e `rotulo_curto`, ao lado, para o cabeçalho que não cabe |
...
| como esconder um campo sem apagar o que já foi gravado | `processo.ts::absorverFormulario` e o princípio 5 |
...
| por que o ofendido não é papel de pessoa, e onde ele mora | decisão **46**, **seção 12** e `migrations/0012_ofendido_vitima.sql` — mais a `0024`, que o religa nos destinos onde a `0012` correu antes da importação |
...
| onde mais existe o par desativar/excluir, e por quê | decisão **66** — `catalogos.ts` e `apuratorio.ts` na tela; `legal_catalogs::repository::delete` e `apuratorio_config::repository::{delete_documento, delete_papel}` no Rust |
| por que o botão Excluir não aparece em toda linha da configuração de apuratório | o `em_uso` de `apuratorio_config::repository::get` — é a mesma pergunta que a FK composta faria ao recusar, e esconder o botão poupa uma recusa previsível. Quem decide continua sendo o banco |
| como artigo e inciso são ordenados, já que são TEXT | `migrations/0023_ordenacao_de_artigo_e_inciso.sql` (cabeçalho), e o campo `ordenacao` dos quatro catálogos jurídicos em `legal_catalogs/domain.rs::CATALOGOS` |
| por que a ordem dos artigos é função e a dos apuratórios é coluna | decisões **67** e **64** — uma é derivada do dado, a outra é escolha do responsável |
| por que o filtro de Catálogos compara com o rótulo, e não com a linha | `catalogos.ts::linhasQueOTermoAlcanca` e `textoDaListagem` (cabeçalhos), e a armadilha da referência filtrada por UUID na seção 7 |
...
| por que o gráfico não vai ao papel | decisão **69** e `graficos/index.ts::tabelasNoLugarDosGraficos` (cabeçalho) — e a fixtura `calibrado-grafico-removido` |
| como se escolhe o tamanho do primeiro bloco de uma tabela impressa | decisão **70**, `dom.ts::blocosDeImpressao` e o `--fragmento=nome:normal:primeiro` de `tools/impressao/gerar-fixturas.ts` |
| por que a segunda tabela de Prazos não tem primeiro bloco próprio | o comentário no `aoImprimir` de `src/telas/prazos.ts` — a posição dela depende do acervo, e valor fixo erraria |
| por que a caixa do gráfico já foi fixada em px para imprimir | histórico: decisão **62** e a rodada 31. Desde a decisão **69** o gráfico não vai ao papel, e as três armadilhas de redimensionamento abaixo deixaram de ser alcançáveis |
...
- [ ] **Status prazo** — gravar a remessa de um apuratório em andamento e com prazo vencido: a coluna vira **Entregue** amarelo, com remessa e vencimento no `title`; limpar a remessa devolve o "Vencido há N dias". "Vence hoje"/"Vence em ≤5 dias" saíram do amarelo e estão laranja
- [ ] **Filtros avançados → Situação** — `Entregue` traz esse apuratório, `Vencido` e `No prazo` não; `Em andamento` continua trazendo. O chip aparece e a paginação bate (listagem e contagem saem de fontes diferentes)
...
- [ ] **Prazos** — o painel carrega, e o apuratório entregue não está em nenhuma das duas tabelas. **O número do cartão "Vencidos" bate com a quantidade de linhas**; conferir também "Com prazo vigente" e "Regulares", e o Painel dizendo o mesmo
- [ ] **Designações** — sem mudança: o entregue com prazo vencido continua em `vencidos`, e os quatro baldes continuam somando o total (decisões 63 e 65)
- [ ] **Usuários → lista**
...
### r) Ordem jurídica, filtro por artigo e "Excluir" nos cadastros

O binário de produção, e o console aberto. **A exclusão é física e não se desfaz** — faça
num banco descartável ou combinado com o responsável.

**A ordem**

- [ ] *Catálogos → Transgressões do RDPM*: a lista abre com o **Art. 15 inteiro**, na
      ordem I, II, III, IV, V, VI… X, XI — e só então o 16 e o 17. Nenhum artigo
      reaparece depois de outro
- [ ] *Catálogos → Infrações do Estatuto*: o **Art. 29 inteiro** em ordem romana, e
      depois o 32. O IX não pode vir antes do V
- [ ] *Catálogos → Artigos do RDPM* e *Infrações penais*: ordem crescente pelo número
- [ ] Cadastrar uma transgressão num artigo novo de **um dígito** (o teste é esse: com
      15/16/17 a ordem alfabética acerta por acaso) e conferir que ela **abre** a lista
- [ ] *Indícios* de um envolvido: abrir o seletor de transgressão e o de infração do
      Estatuto e conferir que a ordem é a **mesma** da tela de Catálogos
- [ ] *Estatísticas*, tabela de enquadramentos: entre itens de mesma contagem, a ordem é
      a jurídica

**O filtro**

- [ ] Em Transgressões, digitar `15` deixa só o Art. 15; `16` só o 16 — é o que não
      funcionava, porque a coluna guarda o UUID do artigo
- [ ] Digitar `Art. 15` funciona igual
- [ ] Digitar parte do texto da transgressão continua funcionando, sem diferenciar
      maiúsculas
- [ ] Em Infrações do Estatuto, `29` e `32` recortam por artigo
- [ ] Em *Municípios e distritos*, digitar o nome do **município pai** alcança os
      distritos dele — coluna de referência, que antes não era pesquisável
- [ ] Digitar `—` não traz nada

**Excluir nos catálogos**

- [ ] Cada linha tem **três** ícones — editar, desativar (ou reativar) e excluir — e só
      o de excluir é vermelho; os três cabem sem rolagem horizontal em 1024px
- [ ] **Com perfil somente leitura, nenhum dos três aparece**
- [ ] A confirmação avisa que **não há como desfazer** e sugere Desativar
- [ ] *Unidades PM*: excluir uma unidade recém-criada e nunca usada tira a linha da
      lista, e ela não volta nem com "Mostrar inativos" marcado
- [ ] *Unidades PM*: excluir uma unidade que já é origem de processo é **recusado**, com
      a frase que manda desativar — e a linha continua lá depois da recusa
- [ ] Desativar continua fazendo o que fazia: a linha fica, riscada, com badge "inativo"
- [ ] Excluir um item da 4ª página **mantém** a página; excluir o último item dela recua
      uma, em vez de mostrar tabela vazia
- [ ] *Perfis de acesso*: excluir o único perfil que administra é recusado

**Excluir na configuração de apuratórios**

- [ ] O ícone de excluir **só aparece** nas linhas com a coluna "em uso" vazia
- [ ] O papel **responsável** não oferece excluir, mesmo sem uso
- [ ] Excluir um documento iniciador nunca usado tira a linha da configuração; o tipo de
      documento continua no catálogo dele
- [ ] Desativar continua disponível em toda linha

**Na Auditoria**

- [ ] As exclusões aparecem com "Excluiu um item de …" / "Excluiu um documento iniciador
      do apuratório" e o **assunto legível**, não um UUID — mesmo com a linha já apagada
- [ ] As desativações continuam como tipo de ação **Alterou**

**A tabela de Apuratórios (decisão 68)**

- [ ] "Ordem no mapa" **não** aparece na tabela — e **continua** no formulário, exigida ao
      criar uma espécie nova
- [ ] Os cabeçalhos de atributo saem sem o verbo ("Julgamento", "Acusação disciplinar",
      "Remessa à comissão"), quebram dentro da própria coluna e **não** invadem a vizinha
- [ ] Parar o mouse num cabeçalho mostra o rótulo completo ("Permite julgamento")
- [ ] O formulário de cadastro continua com os rótulos **inteiros** ao lado de cada campo
- [ ] "Nome" mostra "Conselho de Disciplina" sem reticências
- [ ] "Situação" e "Ações" ficam justas — sem espaço ocioso em volta da tarja e dos três
      botões, e sem cortar o terceiro botão
- [ ] Em monitor de 1920px a tabela cabe inteira, **sem** rolagem horizontal
- [ ] Em ~1366px (ou com zoom) ela **rola** dentro da moldura, e a página não rola na
      horizontal
- [ ] Com perfil somente leitura a coluna de Ações some, e as demais se reacomodam
- [ ] Nos catálogos de uma coluna só (Tipos de documento, Círculos hierárquicos) nada
      mudou: sem rolagem, e a coluna de Nome ocupa a largura disponível
- [ ] Em Transgressões, Artigo e Inciso ficam estreitos e o Texto fica largo

---

### s) A folha 1 dos relatórios, e o gráfico fora do papel (decisões 69 e 70)

O binário de produção. O arnês já cobre a geometria; o que falta aqui é o olho de
quem conhece o documento.

**O gráfico não vai ao papel**

- [ ] Em *Painel*, *Prazos*, *Estatísticas* e *Designações*: deixar o cartão em
      **Gráfico**, clicar em Imprimir/PDF e conferir que o PDF traz a **tabela**
      daquele cartão — nunca o gráfico, nunca um retângulo preto
- [ ] Fechado o diálogo, o cartão volta ao modo **Gráfico** na tela: imprimir não
      pode reescrever a preferência do operador
- [ ] O alternador Gráfico/Tabela **não** aparece no papel
- [ ] Com o cartão já em **Tabela**, o PDF sai igual ao caso anterior
- [ ] O número que estava no gráfico agora é **texto selecionável** no PDF

**A folha 1 começa com tabela**

- [ ] *Usuários*, *Prazos*, *Designações*, *Estatísticas*: a folha 1 traz o
      cabeçalho institucional, o título, os KPIs (onde houver) **e as primeiras
      linhas da tabela**. Nenhuma pode sair só com cabeçalho
- [ ] Nenhuma linha aparece cortada ao pé da folha, e nenhuma **some** entre
      folhas — conferir um registro conhecido do fim de uma folha no começo da
      seguinte
- [ ] O cabeçalho das colunas se repete no topo de cada folha, e **uma vez só**
- [ ] Em *Prazos*, a segunda tabela ("Vencendo em até N dias") pode começar no
      meio de uma folha: é o esperado, e não é defeito
- [ ] Em *Estatísticas*, a tabela de enquadramentos tem as linhas mais altas do
      sistema — conferir que nenhuma passa da margem inferior

**O que não pode ter mudado**

- [ ] *Mapa do Período*: capa, paginação, orientação e conteúdo **idênticos** ao
      de antes. O `tools/impressao/controle-mapa.sh` prova texto e pixel, mas
      vale a conferência visual de uma emissão real

---

## 12. Changelog — as 42 rodadas
...
| 36 | **Datas: ano livre, digitação destravada e `dd/mm/aaaa` em toda tela** | Três queixas de um cadastro de teste, três causas distintas. O calendário não voltava a 2024 porque a ordem cronológica morava em `min`/`max` — e o campo cujo piso é a data de instauração trava o ano em que o processo foi instaurado, inclusive quando é a instauração que está errada. O limite virou `data-limite-*` e a cobrança virou `mensagemDeLimiteDeData`: calendário livre, regra igual, e **nenhuma** validação do Rust ou do banco tocada (decisão 71). O ano não podia ser digitado por causa de um `blur()` no `change`, que existia para fechar o seletor nativo e que dispara no primeiro algarismo do ano — `2` já é o ano `0002`; `ligarCamposDeData` passou a distinguir teclado de mouse, e os dois blocos duplicados de `change` viraram um. E seis pontos ainda mandavam o ISO cru para a tela — a ficha do detalhe, as tabelas de Prazos e Andamentos, a tela de Prazos, o Painel e a ficha do PM —, no mesmo defeito que a rodada 35 corrigiu nos mapas; junto, as **quatro** cópias do formatador viraram uma, `dom.ts::formatarData`, com o vazio como parâmetro. Zero linhas de Rust: os 205 testes passam sem mexer em nenhum. Oito testes novos no Vitest, e os dois que importam falham com o código de antes. O arnês reimprimiu Prazos e a ficha do PM sem mudar folha nem bloco — as fixturas já escreviam `dd/mm/aaaa`, e regeneraram byte a byte iguais —, e o Mapa Mensal saiu idêntico a `HEAD`, texto e pixel. Sem migration, sem comando novo. |
| 37 | **A busca que o markup pedia e a tela não ligava** | Substituir um encarregado abria um `<select>` nativo com os 245 militares ativos, sem busca — enquanto o mesmo campo no cadastro filtra ao digitar. O markup **já pedia** a busca: os cinco selects do detalhe declaram `data-select-pesquisavel` desde que a tela nasceu, e faltava a chamada de `ativarSelectsPesquisaveis`. Uma linha resolve o que se vê; o que ela destapa é maior, porque a tela manipula os selects por DOM nativo em seis pontos e cada um falha **em silêncio** sob o Tom Select — `preencher` da substituição, as três atribuições e o `reset()` do formulário de resultado, a limpeza da penalidade, e sobretudo `garantirOpcaoHistorica`, que insere a solução desativada para o processo de 2019 continuar legível (princípio 6) e que a instância não enxerga: a opção sumiria do menu com o campo abrindo vazio sobre um valor que existe. A ponte é uma só, `sync()`, embrulhada em `dom.ts::sincronizarSelectsPesquisaveis`; o foco passou por `focarCampo`, que também absorveu a lógica duplicada no modal de filtros. **O alcance ficou no detalhe, por decisão**: listas curtas e fixas — mês, esfera, situação, ordenação, SSL — seguem como select comum, que é a mesma escolha já feita no filtro avançado. Três testes novos, os três reprovando o código de antes, e um deles afere o **rótulo visível**, que é onde o defeito aparecia. Sem migration, sem comando novo, sem CSS. |
| 38 | **A SR sem como cadastrar ofendido** | Editar uma SR no banco de produção não oferecia "Adicionar ofendido/vítima": no lugar da seção vinha o aviso de espécie que **não** registra ofendido — sobre as 83 vítimas que a própria SR já exibia. Não era regra nova nem defeito de tela; era a armadilha da seção 7 pela **segunda** vez. A `0012` liga `permite_cadastro_vitima` por `UPDATE`, e `sqlx::migrate!` corre no start do app, portanto **antes** de `importacao/01_catalogos.sql`, que é quem insere as espécies: num destino novo o `UPDATE` acha a tabela vazia, não acerta linha nenhuma sem erro, e tudo nasce no `DEFAULT false`. Foi exatamente o que a `0019` fez com a `ordem` do mapa, e o remédio é o mesmo par: o valor entra no `INSERT` da etapa de importação, para o dado nascer certo, e a **`0024`** alcança o que já foi importado — só LIGA, no molde da `0012`, porque o atributo não é editável por administrador nenhum e não há escolha para atropelar. No banco local nada aparecia: lá a `0012` chegou depois da importação. O guarda novo mora em `tests/importacao.rs` e afirma o **dado** por tipo de apuratório — todo procedimento liga, nenhum processo liga —, com a asserção que exige as duas classes povoadas, senão zero de zero passaria com a coluna fora do `INSERT`; ele reprova o código de antes. E a migration nova obrigou a mexer no `importacao/00_preflight.sql`, que compara o número de migrations por igualdade e que teste nenhum alcança. Migration **0024**. |
| 39 | **Banco para São Paulo** | `sa-east-1` no lugar de `us-east-2`: **215 ms → 80 ms** por ida e volta, medido. A Supabase foi avaliada e descartada — estava na mesma região, e o ganho nunca foi do provedor (decisão 71). Projeto novo `adm-p6-sp`, dados copiados por `pg_dump`/`pg_restore` e conferidos pelo hash da colação, pelos checksums das 24 migrations e pelo `99_conferencia.sql` inteiro. Ohio fica de pé como rollback. No caminho, o `vite.config.ts` que faltava — sem ele o `tauri dev` morria em `ELOOP` no cache do `cargo-xwin`. E uma espera a menos na abertura do formulário: catálogos e ficha eram dois `await` em fila sem dependência entre si |
| 40 | **Datas que não aceitavam digitação, e o menu que voltava ao topo** | Dois campos de data — os das duas telas de prorrogação — nunca tinham recebido o conserto da rodada anterior: usavam uma cópia manual do `change`, com `blur()` sem guarda de teclado. Os 8 campos do app passaram a sair do mesmo `ligarCamposDeData` (callback agora opcional), e os filtros e a data de substituição ganharam junto o fechamento do seletor. No menu, `preservarRolagem` guarda o `scrollTop` da `.sidebar` no redesenho do `shell()`: clicar num dos 26 catálogos deixa de devolver o menu ao topo |
| 41 | **As mensagens que o usuário lê** | Auditoria das três fontes de mensagem. `AppError::Domain` — a maior, 82 mensagens — não tinha teste e acumulara **36** desvios (minúsculas, sem pontuação, 10 sem acento), todas indo verbatim para a tela; reescritas, e agora cobradas por `tests/mensagens_de_erro.rs`, que varre o fonte como o `sql_prepare.rs`. O achado maior não era redação: `max_envolvidos` **não era validado** em lugar nenhum, e o gatilho adiado respondia "Os dados informados não atendem a uma regra do cadastro" — agora a frase diz o limite e quantos foram informados, antes do SQL. Mais três constraints alcançáveis sem frase (a família da vítima, esquecida na 0012) e três testes que prendiam o defeito em vez de denunciá-lo |
| 42 | **Aviso por e-mail ao encarregado** | Botão na tela do apuratório abre modal com os três avisos, mostra o texto final já preenchido e envia — prévia obrigatória porque o corpo vem de catálogo editável e e-mail enviado não volta. Migration `0025`: `policiais_militares.email` (opcional), o catálogo `mensagens_email` com as três mensagens semeadas, e `configuracao_email` de linha única. Destinatário por `COALESCE(pm.email, u.email)`; três faltas, três frases. `lettre` com rustls, 587/STARTTLS ou 465/TLS conforme a porta, e `multipart/alternative`: moldura HTML com estilo inline para quem tem HTML, texto puro para quem não tem. No caminho: `TipoColuna::TextoLongo` no motor de catálogos (o CRUD não tinha `<textarea>`), `Catalogo::so_edicao` para não oferecer botão que só erra, e o preflight da importação de 24 para 25 |

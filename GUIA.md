...
| 42 | **Aviso por e-mail ao encarregado** | Botão na tela do apuratório abre modal com os três avisos, mostra o texto final já preenchido e envia — prévia obrigatória porque o corpo vem de catálogo editável e e-mail enviado não volta. Migration `0025`: `policiais_militares.email` (opcional), o catálogo `mensagens_email` com as três mensagens semeadas, e `configuracao_email` de linha única. Destinatário por `COALESCE(pm.email, u.email)`; três faltas, três frases. `lettre` com rustls, 587/STARTTLS ou 465/TLS conforme a porta. No caminho: `TipoColuna::TextoLongo` no motor de catálogos (o CRUD não tinha `<textarea>`), `Catalogo::so_edicao` para não oferecer botão que só erra, e o preflight da importação de 24 para 25 |
